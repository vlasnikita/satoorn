...
var UnitPay = function(params) {
    ...
    this.createWidget = function(params) {
        ...
        let iframeFormUrl = 'https\u003A\/\/widget.unitpay.money\/pay\/\u0040PUBLIC_ID\u0040';

        iframeFormUrl = iframeFormUrl.replace('@PUBLIC_ID@', publicKey) + '?';

        var iframe = document.createElement("iframe");
        var billingCode = 'card';
        ...
        if (params['paymentType'] === 'sms' || params['paymentType'] === 'applepay') {
            delete params['paymentType'];
        }

        if (params['paymentType']) {
            billingCode = params['paymentType'];
        ...
        if (isMobilePhone()) {
            var paymentFormUrl = 'https\u003A\/\/unitpay.money\/pay\/\u0040PUBLIC_ID\u0040\/\u0040BILLING_CODE\u0040';
            paymentFormUrl = paymentFormUrl.replace('@PUBLIC_ID@', publicKey);
            paymentFormUrl = paymentFormUrl.replace('@BILLING_CODE@', billingCode) + '?';
...
export const processPaymentInNewTab = (amount, transaction, desc) => {
    const signature = getFormSignature(amount, transaction, desc)
    // const url = `${HOST}${PUBLIC_KEY}?sum=${amount}&account=${transaction}&desc=${desc}&signature=${signature}&currency=RUB&domainName=unitpay.money`
    // openInNewTab(url)
